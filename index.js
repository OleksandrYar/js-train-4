...
function padString(original, length, padWith, side) {
  ...
  if (side === "start") {
    // Використовуємо метод padStart(), щоб доповнити рядок original до певної довжини length, вставляючи символи padWith на початку
    return original.padStart(length, padWith);
  } else if (side === "end") {
    // Використовуємо метод padEnd(), щоб доповнити рядок original до певної довжини length, вставляючи символи padWith на в кінці
    return original.padEnd(length, padWith);
  } else {
    // Якщо бік не вказано або вказано неправильно, повертаємо повідомлення про помилку
...
function trimString(original, side) {
  ...
  // Якщо значення аргументу "side" не є ні "start", ні "end", ні "both", повертаємо повідомлення про помилку
  else {
...
function convertCase(original, caseType) {
  ...
  // Якщо значення аргументу "caseType" не є ні "upper", ні "lower", повертаємо повідомлення про помилку
  else {
